 ...
 * Simulates forwarding the video to one contact — "Tita" 🤖.
 * Selecting Tita and pressing "Kirim" triggers the SHARE action in the store.
 */
  ...
  return (
    <BottomSheet isOpen={isOpen} onClose={handleClose}>
      <div className="min-h-[350px] flex flex-col">
        ...
        >
          {/* Avatar */}
          <div className="w-12 h-12 rounded-full bg-indigo-500/20 flex items-center justify-center shrink-0 border border-indigo-400/30 overflow-hidden">
            <Image src="/assets/tita_beach.png" alt={FRIEND.name} width={48} height={48} className="object-cover w-full h-full" />
          </div>
        ...
        <motion.button
          ...
          className={`w-full mt-4 flex items-center justify-center gap-2 rounded-full py-3 font-display font-bold text-sm ${selected ? "btn-primary" : "bg-white/5 text-white/30 transition-all duration-200"
            }`}
          style={{
            opacity: sent ? 0.7 : 1,
          }}
